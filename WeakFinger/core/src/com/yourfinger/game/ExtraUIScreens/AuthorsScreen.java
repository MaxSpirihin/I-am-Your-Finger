...
public class AuthorsScreen implements Screen {
	...
	final Color LINK_PRESSED_COLOR = new Color(0, 0, 0.6f, 1);
	...
	private void AddRateButton(Table table) {
		...
		btnRate.addListener(new ClickListener() {
			...
			public boolean touchDown(InputEvent event, float x, float y,
				...
				btnRate.getStyle().fontColor = LINK_PRESSED_COLOR;
				return true;
			};
			...
			public void touchUp(InputEvent event, float x, float y,
				...
				if ((x < btnRate.getWidth()) && (y < btnRate.getHeight())
						&& (x > 0) && (y > 0))
					Gdx.net.openURI("https://play.google.com/store/apps/details?id=com.yourfinger.game.android");
				btnRate.getStyle().fontColor = LINK_COLOR;
			}
		});
	...
	private void AddPhotoAndDevInfo(Table table) {
		...
		btnVk.addListener(new ClickListener() {
			...
			public boolean touchDown(InputEvent event, float x, float y,
					int pointer, int button) {
				Gdx.input.vibrate(20);
				btnVk.getStyle().fontColor = LINK_PRESSED_COLOR;
				return true;
			};
			...
			public void touchUp(InputEvent event, float x, float y,
				...
				btnVk.getStyle().fontColor = LINK_COLOR;
			}
		});
	...
	private void AddHelpAboutFrom(Table table) {
		...
		// помогали
		Label lblHelp = new Label(game.GetString("help"), labelStyle);
		lblHelp.setAlignment(Align.center);
		lblHelp.setWidth(Gdx.graphics.getWidth());
		lblHelp.setWrap(true);
		table.add(lblHelp).padBottom(Gdx.graphics.getHeight() * 0.02f);
		table.row();
	}
	...
	private void AddLastAndYear(Table table) {
		...
		btnLast.addListener(new ClickListener() {
			...
			public boolean touchDown(InputEvent event, float x, float y,
					int pointer, int button) {
				Gdx.input.vibrate(20);
				btnLast.getStyle().fontColor = LINK_PRESSED_COLOR;
				return true;
			};
			...
			public void touchUp(InputEvent event, float x, float y,
				...
				btnLast.getStyle().fontColor = LINK_COLOR;
			}
		});
